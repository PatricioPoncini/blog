...
summary: 'Descubrí por qué el testing es indispensable en el desarrollo profesional. Aprendé las diferencias entre Unit e Integration tests y mirá ejemplos prácticos de cómo implementarlos en TypeScript para dormir tranquilo cuando subas tu código a producción.'
date: '2025-11-24'
draft: false
tags:
  - Desarrollo
...
- Verificar el correcto envío de datos hacia a un endpoint.
- Comprobar la persistencia en la base de datos de información.
- Validar la respuesta de un endpoint ante datos que no son correctos.
- Entre otros
...
- **Detección temprana de errores (Bugs):** Muchas veces al escribir el test nos damos cuenta de que nos faltó una validación o que la lógica estaba floja. Es mejor encontrar el bug mientras escribimos los casos de test a que recibamos el reporte de un usuario molesto.
- **Refactorización segura y deploy con confianza:** Esto es, para mí, la clave. Si tenés una buena suite de tests cubriendo tu lógica, podés hacer cambios, optimizar o refactorizar código con la tranquilidad de que, si rompiste algo, los tests te deberían a avisar antes de subir a producción.

## Tipos de casos de test

### Unit test

Los **unit tests** (o tests unitarios) verifican una pieza pequeña y aislada del código sin depender de servicios externos como una base de datos o una API por ejemplo.

Como en estas pruebas solo queremos comprobar la lógica interna, es común usar **mocks** (objetos falsos que reemplazan servicios reales para controlar su comportamiento durante el test).

Por ejemplo, imaginemos que tenemos un servicio sobre el cual queremos desarrollar casos de test, y contiene un cliente de Redis con la siguiente función:

```ts
static lpush(key: string, value: string): Promise<number> {
...
La desventaja es que estamos simulando el comportamiento del servicio, por lo que el mock solo cubre aquello que nosotros imaginamos. Es decir: si el servicio real se comporta distinto en algún escenario que no mockeamos, el unit test no lo va a detectar. Otro riesgo es que si la API del servicio cambia, el caso de test seguiría pasando pero al subir el código a producción nos podríamos encontrar con un comportamiento inesperado.

### Integration test

Los **integration tests** (o tests de integración) son una forma de testear donde varios componentes de una aplicación trabajan juntos para ver si el sistema completo se comporta como debería.

Por ejemplo, imaginemos un endpoint que busca un usuario en la base de datos mediante una consulta. Si el usuario no existe, devuelve un 404 con un mensaje de error. Un test de integración podría levantar una base de datos real (por ejemplo con Docker), conectarse a ella y validar qué pasa cuando realmente buscamos un usuario que no existe. El objetivo es validar la interacción real entre los componentes y los servicios externos.
...
- Guarde el mensaje en una base de datos real (Redis en este caso)

Como usaremos un servicio real, lo primero es levantarlo. Lo más cómodo es hacerlo con un contenedor Docker, así no tocamos nuestra base local y siempre tenemos un entorno limpio para testear. Una vez levantado Redis, conectamos nuestro cliente y borramos el contenido de la key **"messages"**. Esto es **importante**: los tests de integración siempre deben empezar desde un estado limpio.
Si no, los resultados pueden fallar o variar dependiendo del orden de ejecución. Para esto utilizamos la función **_*beforeEach*_** donde definimos las acciones que deben ejecutarse antes de cada prueba, como limpiar la base de datos.
...
A medida que aumenten los casos de test, también se vuelven más variados y/o complejos. Por ejemplo podemos validar que el endpoint rechace peticiones inválidas. El siguiente caso prueba que si enviamos un número en vez de un string, el servidor debe:
...
En resumen: los tests de integración son ideales para asegurarte de que el sistema realmente funcione cuando sus piezas colaboran en el uso real. Son más lentos que los unit tests pero también más cercanos al comportamiento que se espera en producción.

## Conclusión

A medida que crecés como desarrollador, te das cuenta de que los tests no son un "extra", sino una necesidad. No se trata solo de escribir código, sino de construir software robusto que soporte miles de ejecuciones sin fallar. Incorporar testing no solo te da paz mental al momento de subir tu código a producción, sino que eleva significativamente la calidad profesional de tu trabajo.

Para bajar toda esta teoría a tierra, armé un repositorio con los **ejemplos anteriores y muchos más**. Es un servicio backend simple utilizando **TypeScript, Bun y Redis** que expone endpoints para guardar y leer mensajes. Lo interesante es que ahí vas a encontrar aplicados todos los conceptos que vimos hoy.
